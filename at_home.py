...
def process_video(audio_path, image_path, output_folder, file_name):
    ...
    try:
        ...
        for i, subtitle in enumerate( subtitles):
            ...
            if i == 0:
            ...
            elif i % 2 ==1 and  ( i !=3 or file_name !=  '05-location-2')  :   
                # left 
                ...
                if  ( i == 1 or i == 5 )  and file_name == '05-location-2'  :
                    rows_pixes += text_clip_en.h
                    rows = round(rows_pixes / screen_height, 3) + space_two_line
            else:    
                # right
                ...
                if  i == 2 and file_name == '04-location':
                    text_clip_en = text_clip_en.set_position((0.4, rows), relative=True).set_start(start_time).set_end(end_all_video_time)
                elif file_name == '05-location-2':
                    text_clip_en = text_clip_en.set_position((0.4, rows), relative=True).set_start(start_time).set_end(end_all_video_time)    
                else:
